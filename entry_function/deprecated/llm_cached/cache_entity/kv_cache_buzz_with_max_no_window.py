...
class FixedStrideKVCacheWithMaxNoWindow:
    def __init__(
            self,
            sink_size=4,
            stride_size=4,
        ...
        print(
            f"FixedStrideKVCache with local max no window, sink size, stride size, sample threshold: "
            f"{sink_size}, "
            f"{stride_size}, "
            f"{sample_threshold} "
        )
    ...
    def get_sink_other(self, new_coming_words):
    ...
    def deal_new_sample_with_local_max(self, k, v, attn_score):
        ...
        hh_score = hh_score[:, self.sink_size + self.cur_sample_size:]
    ...
    def __call__(self, current_words, attn_scores):
        if current_words is None:
            ...
            if cur_cache_size <= self.sink_size + self.sample_threshold:
                return current_words
            sink, other = self.get_sink_other(current_words)
            old_sample = [
                [
            ...
            return self.concat_three_parts(sink, cur_sample, [])
